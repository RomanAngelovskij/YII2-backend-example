...
use app\models\WorksheetsamplesQuestions;
use Yii;
...
class QuestionsController extends Controller{
    ...
    public function actionIndex()
    {
        ...
        $query = WorksheetsamplesQuestions::find();
    ...
    {
        $filter = Yii::$app->request->get();
        $model = new WorksheetsamplesQuestions();
        if (isset($filter['worksheetsampleId']) && !empty((int)$filter['worksheetsampleId'])){
            $model->worksheetsample_id = $filter['worksheetsampleId'];
        }
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect('/questions?' . http_build_query($filter));
        }
    ...
    {
        $filter = Yii::$app->request->get();
        $model = WorksheetsamplesQuestions::findOne($id);
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect('/questions?' . http_build_query($filter));
        }
    ...
    public function actionCopy()
    {
        ...
        $field = WorksheetsamplesQuestions::findOne(Yii::$app->request->get('fieldId'));
    ...
    public function actionSort()
    {
        ...
        foreach ($order as $orderPosition => $id){
            $item = WorksheetsamplesQuestions::findOne(['id' => $id]);
